/**
 * BotHive multi-tenant control panel server.
 *
 * Features:
...
const PANEL_HTML = path.join(__dirname, 'web/multibot.html');
const PANEL_ASSETS_DIR = path.join(__dirname, 'web/assets');
...
const FIRST_ADMIN_EMAIL = process.env.BOTHIVE_ADMIN_EMAIL || process.env.BM_ADMIN_EMAIL || 'admin';
const configuredAdminPassword = process.env.BOTHIVE_ADMIN_PASSWORD || process.env.BM_ADMIN_PASSWORD || '';
...
if (firstAdmin && firstAdmin.ok && !configuredAdminPassword) {
    console.log('[BotHive] First admin created. Set BOTHIVE_ADMIN_PASSWORD before production.');
    console.log(`[BotHive] Login: ${FIRST_ADMIN_EMAIL}`);
    console.log(`[BotHive] Temporary password: ${FIRST_ADMIN_PASSWORD}`);
}
...
async function handleHttp(req, res, state) {
    ...
    if (req.method === 'GET' && p.startsWith('/assets/')) {
        const relativePath = p.slice('/assets/'.length);
        const assetPath = path.resolve(PANEL_ASSETS_DIR, relativePath);
        const assetRoot = PANEL_ASSETS_DIR + path.sep;
        const contentTypes = {
        ...
        if (!relativePath || !assetPath.startsWith(assetRoot) || !contentType || !fs.existsSync(assetPath)) {
    ...
    if (req.method === 'POST' && p === '/api/login') {
        ...
        if (user) {
            ...
            const secure = process.env.BOTHIVE_COOKIE_SECURE === '1' ? '; Secure' : '';
            res.writeHead(200, {
...
async function start(port = 3123) {
    ...
    server.listen(port, host, () => {
        console.log('');
        console.log('BotHive Control Panel');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
