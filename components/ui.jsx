...
export function Button({ children, variant = 'secondary', size = 'md', className, loading, disabled, ...props }) {
  const variants = {
    primary: 'border-white bg-white text-black font-bold shadow-[0_4px_24px_rgba(255,255,255,0.3)] hover:bg-white/90 active:scale-[0.96]',
    secondary: 'border-white/15 bg-white/[0.08] text-white font-semibold backdrop-blur-2xl hover:border-white/30 hover:bg-white/[0.14] active:scale-[0.96]',
    ghost: 'border-transparent bg-transparent text-white/70 hover:bg-white/[0.10] hover:text-white active:scale-[0.96]',
    danger: 'border-white/25 bg-white/[0.05] text-white hover:border-white hover:bg-white hover:text-black active:scale-[0.96]',
    success: 'border-white/25 bg-white/[0.12] text-white hover:bg-white/[0.22] active:scale-[0.96]',
  };
  const sizes = {
    sm: 'h-9 min-h-[36px] gap-2 rounded-[13px] px-3.5 text-[13px]',
    md: 'h-11 min-h-[44px] gap-2.5 rounded-[15px] px-5 text-[14px]',
    lg: 'h-13 min-h-[52px] gap-3 rounded-[17px] px-7 text-[16px]',
  };
  return (
    <button
      className={cn(
...
export function IconButton({ label, children, className, ...props }) {
  return (
    <button
      ...
      title={label}
      className={cn(
        'inline-flex h-11 w-11 min-h-[44px] min-w-[44px] items-center justify-center rounded-[14px] border border-white/12 bg-white/[0.06] text-white/80 backdrop-blur-2xl transition-all duration-200 hover:border-white/30 hover:bg-white/[0.14] hover:text-white active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30',
        className
      )}
      {...props}
    >
...
export function PageHeader({ eyebrow, title, description, actions }) {
  return (
    <header className="anim-rise flex flex-col gap-4 border-b border-white/[0.08] pb-6 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        {eyebrow && (
          <p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/60">
        ...
        {description && (
          <p className="mt-2.5 max-w-3xl text-sm leading-relaxed text-white/70 sm:text-base">{description}</p>
        )}
      </div>
      {actions && <div className="flex shrink-0 flex-wrap items-center gap-3 pt-1">{actions}</div>}
    </header>
  );
}
...
export function StatCard({ label, value, hint, icon: Icon, tone = 'default' }) {
  const dots = {
    default: 'bg-white/40',
    blue: 'bg-white/90',
    green: 'bg-white shadow-[0_0_10px_rgba(255,255,255,1)]',
    red: 'bg-white anim-pulse',
    amber: 'bg-white/80',
  };
  return (
    <Panel className="group relative flex min-h-32 flex-col justify-between overflow-hidden p-5 transition-all duration-300">
      <div className="flex items-start justify-between gap-3">
        <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-white/60">
          <span className={cn('h-2 w-2 shrink-0 rounded-full', dots[tone] || dots.default)} />
          <span className="truncate">{label}</span>
        </p>
        {Icon && (
          <span className="shrink-0 rounded-[14px] border border-white/10 bg-white/[0.06] p-2.5 text-white/60 transition-all duration-300 group-hover:border-white/30 group-hover:text-white">
            <Icon className="h-5 w-5" />
          </span>
        )}
      </div>
      <div className="mt-3">
        <p className="tnum text-3xl font-black tracking-tight text-white sm:text-4xl">{value}</p>
        {hint && <p className="mt-1.5 truncate text-xs font-medium text-white/50">{hint}</p>}
      </div>
    </Panel>
  );
}
...
export function StatusBadge({ status }) {
  ...
  const tones = {
    running: { wrap: 'border-white/40 bg-white/[0.18] text-white shadow-[0_0_12px_rgba(255,255,255,0.25)]', dot: 'bg-white shadow-[0_0_8px_rgba(255,255,255,1)]' },
    online: { wrap: 'border-white/40 bg-white/[0.18] text-white shadow-[0_0_12px_rgba(255,255,255,0.25)]', dot: 'bg-white shadow-[0_0_8px_rgba(255,255,255,1)]' },
    done: { wrap: 'border-white/30 bg-white/[0.12] text-white', dot: 'bg-white' },
    stopped: { wrap: 'border-white/15 bg-white/[0.05] text-white/60', dot: 'bg-white/40' },
    offline: { wrap: 'border-white/15 bg-white/[0.05] text-white/60', dot: 'bg-white/40' },
    cancelled: { wrap: 'border-white/15 bg-white/[0.05] text-white/60', dot: 'bg-white/40' },
    pending: { wrap: 'border-white/20 bg-white/[0.08] text-white/80', dot: 'bg-white/80 anim-pulse' },
    partial: { wrap: 'border-white/20 bg-white/[0.08] text-white/80', dot: 'bg-white/80' },
    running_job: { wrap: 'border-white/30 bg-white/[0.12] text-white', dot: 'bg-white anim-pulse' },
    error: { wrap: 'border-white/50 bg-white/[0.12] text-white', dot: 'bg-white anim-ring' },
    failed: { wrap: 'border-white/50 bg-white/[0.12] text-white', dot: 'bg-white anim-ring' },
  };
  const tone = tones[normalized] || tones.stopped;
  return (
    <span className={cn('inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-wider backdrop-blur-xl', tone.wrap)}>
