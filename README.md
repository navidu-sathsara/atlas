...
BotHive is a multi-tenant control panel for hosting isolated Minecraft bot
...
```bash
npm install
export BOTHIVE_ADMIN_PASSWORD='replace-with-a-long-random-password'
npm start
```

The panel listens on `127.0.0.1:3000` by default. Set `HOST=0.0.0.0` only when
the process must accept direct network traffic. For production, place BotHive
behind an HTTPS reverse proxy and set `BOTHIVE_COOKIE_SECURE=1`.

Set `BOTHIVE_DATA_DIR` to a persistent private volume in production. This keeps
tenant data outside the application checkout and also makes upgrades simpler.

If no admin password is configured on the first boot, BotHive creates a random
...
```bash
npm start       # start the BotHive panel
npm run start:bot
